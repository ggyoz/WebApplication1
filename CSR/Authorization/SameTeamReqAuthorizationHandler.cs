...
namespace CSR.Authorization
{
    public class SameTeamReqAuthorizationHandler : AuthorizationHandler<SameTeamRequirement, ReqInfo>
    {
            ...
            ReqInfo resource) // 'resource' 매개변수로 검사할 게시글(ReqInfo) 객체가 들어옵니다.
        {
            ...
            string? userCorCd = httpContext.Session.GetString("CorCd");
            string? userTeamCd = httpContext.Session.GetString("TeamCd");
            string? userOfficeCd = httpContext.Session.GetString("OfficeCd");
            string? userDeptCd = httpContext.Session.GetString("DeptCd");

            // 경영층 지시사항 , 개선과제일 경우
            if( !string.IsNullOrEmpty(userDeptCd) && userCorCd == "1200" ){ 
                
                if( resource.DEPTCD == "CM00" || resource.DEPTCD == "ST00"){
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            if( userRole == "R1"){
                ...
                // 팀장
                if (!string.IsNullOrEmpty(userTeamCd))
                {
                    if (userTeamCd == resource.TEAMCD) context.Succeed(requirement);
                }
                // 실장
                else if (!string.IsNullOrEmpty(userOfficeCd))
                {
                    if (userOfficeCd == resource.OFFICECD) context.Succeed(requirement);
                }
                // 3. 사업부장
                else if (!string.IsNullOrEmpty(userDeptCd))
                {
                    if (userDeptCd == resource.DEPTCD) context.Succeed(requirement);
                }
