CREATE TABLE TB_REQ_INFO (
    ...
    USEYN VARCHAR2(1) DEFAULT 'N' NOT NULL,
    CONSTRAINT PK_TB_REQ_INFO PRIMARY KEY (REQID)
);

COMMENT ON TABLE TB_REQ_INFO IS '요청사항';
COMMENT ON COLUMN TB_REQ_INFO.REQID IS '요청번호';
COMMENT ON COLUMN TB_REQ_INFO.PARENTID IS '상위요청번호';
...
COMMENT ON COLUMN TB_REQ_INFO.REQENDDATE IS '완료요청(예정)일';
...
COMMENT ON COLUMN TB_REQ_INFO.REQHISTORY IS '조치내역(히스토리)';
COMMENT ON COLUMN TB_REQ_INFO.REG_DATE IS '등록일';
COMMENT ON COLUMN TB_REQ_INFO.REG_USERID IS '등록자ID';
