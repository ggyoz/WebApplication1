...
CREATE TABLE TB_REQ_HIST (
    ...
    UPDATE_DATE     DATE, -- Added based on previous schema
    UPDATE_USERID   VARCHAR2(10), -- Added based on previous schema
    USEYN           VARCHAR2(1)     DEFAULT 'Y' NOT NULL,
    CONSTRAINT PK_TB_REQ_HIST PRIMARY KEY (HISTORYID)
);

COMMENT ON TABLE TB_REQ_HIST IS '요청사항 관리 이력 테이블';
...
-- Sequence for a primary key
CREATE SEQUENCE SEQ_TB_REQ_HIST
START WITH 1
INCREMENT BY 1;

-- Trigger for a primary key
CREATE OR REPLACE TRIGGER TRG_TB_REQ_HIST_PK
BEFORE INSERT ON TB_REQ_HIST
FOR EACH ROW
BEGIN
    IF :NEW.HISTORYID IS NULL THEN
        SELECT SEQ_TB_REQ_HIST.NEXTVAL
        INTO :NEW.HISTORYID
        FROM DUAL;
    END IF;
END;
