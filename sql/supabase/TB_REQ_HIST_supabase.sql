-- TB_REQ_HIST Table (요청사항 히스토리) for Supabase (PostgreSQL)
...
CREATE TABLE public.TB_REQ_HIST (
    ...
    CONSTRAINT PK_TB_REQ_HIST PRIMARY KEY (HISTORYID, REQID)
);

-- Comments
COMMENT ON TABLE public.TB_REQ_HIST IS '요청사항 히스토리 테이블';
...
COMMENT ON COLUMN public.TB_REQ_HIST.DUEDATE IS '처리기한';
COMMENT ON COLUMN public.TB_REQ_HIST.EXPECTDATE IS '완료예정일';
COMMENT ON COLUMN public.TB_REQ_HIST.STARTDATE IS '실제 시작일';
COMMENT ON COLUMN public.TB_REQ_HIST.ENDDATE IS '실제 종료일';
COMMENT ON COLUMN public.TB_REQ_HIST.REQTYPE IS '요청유형';
COMMENT ON COLUMN public.TB_REQ_HIST.SYSTEMCD IS '대상시스템';
...
COMMENT ON COLUMN public.TB_REQ_HIST.OFFICECD IS '실 코드';
